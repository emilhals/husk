...
print_home(Screen *screen)
{
	...
	for (int i = 0; i < n; i++) {
		if (screen->menu_position == i) {
			mvprintw(center_y, (screen->x - strlen(items[i])) / 2,
			         "-> %s <-", items[i]);
		} else {
			mvprintw(center_y, (screen->x - strlen(items[i])) / 2,
			         "   %s   ", items[i]);
...
print_add_card(Screen *screen)
{
	...
	WINDOW *front_win;
	WINDOW *back_win;
	front_win = newwin(screen->y / 2, screen->x / 2, screen->y / 4,
	                   screen->x / 4);

	back_win = newwin(screen->y / 2, screen->x / 2, screen->y / 4,
	                  screen->x / 4);
	...
	create_card(front_text, back_text);
...
void
print_view_cards(Screen *screen)
{
	read_cards();
	int n;
	Card *card = list_cards("CO2");
...
void
print_info(Screen *screen)
{
	WINDOW *bottom;

	bottom = newwin(screen->y / 2, screen->x / 2, screen->y / 4,
	                screen->x / 4);
